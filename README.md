...
## 🧠 算法原理
LeNet-5 由 Yann LeCun 于 1998 年提出，是首个成功商用的卷积神经网络，奠定了现代 CNN 的基础架构。网络核心由「卷积层 + 池化层 + 全连接层」交替堆叠构成，依靠局部感受野、权值共享、池化下采样三大机制，自动提取图像层级化特征。

**网络层级结构**：
1. **输入层**：1×32×32 单通道灰度图像
2. **C1 卷积层**：6 个 5×5 卷积核，输出 6@28×28 特征图
3. **S2 最大池化层**：2×2 池化窗口，输出 6@14×14 特征图
4. **C3 卷积层**：16 个 5×5 卷积核，输出 16@10×10 特征图
5. **S4 最大池化层**：2×2 池化窗口，输出 16@5×5 特征图
6. **C5 卷积层**：120 个 5×5 卷积核，输出 120 维特征向量
7. **F6 全连接层**：120 → 84 维特征映射，ReLU 激活
8. **输出层**：84 → 10 维，对应 0~9 十类数字分类

---

## 🛠️ 环境要求与依赖安装
### 硬件环境
- 最低配置：Intel i5 及以上 CPU、8GB 运行内存（纯 CPU 训练）
- 推荐配置：NVIDIA 显卡（4GB 及以上显存），支持 CUDA 加速
- 操作系统：Windows 10/11、Linux、macOS 均可运行
